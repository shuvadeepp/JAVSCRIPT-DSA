...
/* 3. [] == [] → false (Each array is a new reference, so they are not the same object in memory.)
      [] == false → true (Empty array is coerced to an empty string "", which is then coerced to 0, and 0 == false is true.)
...
/* 13. ![] → false (because an empty array is truthy, so ![] becomes false).
       [] == false → true (because [] is coerced to an empty string "", which is then coerced to 0, and 0 == false is true). 
*/
// console.log([] == ![]);
